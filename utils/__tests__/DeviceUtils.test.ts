...
describe("DeviceUtils", () => {
  ...
  const mockDimensions = (width: number, height: number): ScaledSize => ({
    width,
    height,
    scale: 2,
    fontScale: 1
  ...
  describe("getDeviceType", () => {
    it("应该在宽度 >= 1024 时返回 tv", () => {
      mockedDimensions.get.mockReturnValue(mockDimensions(1024, 768));
      expect(DeviceUtils.getDeviceType()).toBe("tv");
    });

    it("应该在宽度 >= 768 且 < 1024 时返回 tablet", () => {
      mockedDimensions.get.mockReturnValue(mockDimensions(768, 1024));
      expect(DeviceUtils.getDeviceType()).toBe("tablet");
    });

    it("应该在宽度 < 768 时返回 mobile", () => {
      mockedDimensions.get.mockReturnValue(mockDimensions(375, 812));
  ...
  describe("isTV", () => {
    it("应该在 TV 设备上返回 true", () => {
      mockedDimensions.get.mockReturnValue(mockDimensions(1920, 1080));
  ...
  describe("isLandscape", () => {
    it("应该在横屏模式下返回 true", () => {
      mockedDimensions.get.mockReturnValue(mockDimensions(812, 375));
    ...
    it("应该在宽高相等时返回 false", () => {
      mockedDimensions.get.mockReturnValue(mockDimensions(500, 500));
      expect(DeviceUtils.isLandscape()).toBe(false);
    });
  });
